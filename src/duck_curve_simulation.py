...
Builds on analyze_duck_curve() and predict_curtailment_strategy() from
duck_curve_analysis.py and adds:
...
def _ramp_thresholds(capacity_mw):
    """Return (high_threshold, curtail_threshold) in MW/h for this farm size."""
    return capacity_mw * HIGH_RAMP_FRACTION, capacity_mw * CURTAIL_RAMP_FRACTION
...
def ghi_to_pv_power(ghi_wm2: np.ndarray,
    ...
    # Panel area is derived from the capacity actually requested, so
    # --capacity scales generation correctly even mid-run.
    panel_area_m2 = (capacity_mw * 1e6) / (1000 * PANEL_EFFICIENCY)
...
def simulate_day(day_df: pd.DataFrame, capacity_mw: float = SOLAR_FARM_MW) -> dict:
    """
    Run the full duck-curve simulation for a single 24-hour day.

    Args:
        day_df: DataFrame with 24 rows, columns: timestamp, ghi
        capacity_mw: Installed PV farm capacity (MW), used both for the
            GHI→power conversion and to scale the ramp-rate thresholds.
    ...
    pv_power  = ghi_to_pv_power(ghi, temp, capacity_mw=capacity_mw)
    ...
    # Duck-curve analysis, with ramp thresholds scaled to this farm's capacity
    # (the GHI-calibrated defaults in duck_curve_analysis are meaningless here)
    high_thr, low_thr = _ramp_thresholds(capacity_mw)
    analysis     = analyze_duck_curve(pv_power.tolist(), high_ramp_threshold=high_thr)
    curtailment  = predict_curtailment_strategy(analysis, pv_power.tolist(),
                                                 high_threshold=high_thr, low_threshold=low_thr)
    ...
    return dict(
        ...
        high_ramp_threshold    = high_thr,
        curtail_ramp_threshold = low_thr,
    )
...
def simulate_all_days(df: pd.DataFrame, capacity_mw: float = SOLAR_FARM_MW) -> list:
    ...
    for date, group in grouped:
        ...
        try:
            results.append(simulate_day(group_24, capacity_mw=capacity_mw))
        except Exception:
            continue
...
def pick_representative_days(results: list) -> dict:
    """
...
def plot_simulation(day: dict,
    ...
    high_thr = day["high_ramp_threshold"]
    ax1.axhline(0, color=LGRAY, lw=0.8, linestyle="-")
    ax1.axhline( high_thr, color=RED,    lw=1.2,
...
def print_report(day: dict, all_results: list):
...
def main():
    ...
    all_results = simulate_all_days(df, capacity_mw=args.capacity)
